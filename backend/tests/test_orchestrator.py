...
def test_attack_response_log_keeps_failed_and_successful_approaches(tmp_path):
    path = tmp_path / "attack_responses.json"
...
def test_attack_response_log_appends_with_unique_response_ids(tmp_path):
    ...
    # Archives written by schema v1 before agent_backend existed remain readable.
    legacy_archive = json.loads(path.read_text())
    legacy_archive["responses"][0].pop("agent_backend")
    path.write_text(json.dumps(legacy_archive))
